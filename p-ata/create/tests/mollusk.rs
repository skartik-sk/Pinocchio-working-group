...
fn setup_mollusk() -> Mollusk {
    let mut mollusk = Mollusk::new(&PROGRAM_ID, "target/deploy/create");
...
#[test]
fn test_idempotent_existing_ata_fails() {
    let mollusk = setup_mollusk();
    let token_program = spl_token_interface::id();
    ...
    mollusk.process_and_validate_instruction(&ix, &accounts, &[Check::err(
        solana_program_error::ProgramError::IllegalOwner,
    )]);
}
...
#[test]
fn test_idempotent_existing_ata_token2022_fails() {
    let mollusk = setup_mollusk();
    let token_program = spl_token_2022_interface::id();
